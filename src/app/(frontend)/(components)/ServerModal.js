...
const ServerModal = ({ onClose, userId }) => {
  ...
  const createServer = () => {
    if (serverCategory && serverName && serverName.length <= 20) {
      ...
      const newServer = {
        ...
        serverIcon: serverIcon ? serverIcon : null,
        serverChannels:
          serverCategory === "Work" ? workServerChannels : gamingServerChannels,
        members: [userId],
      };
      // onClose();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center text-black z-30">
      <div className="bg-blue-50 rounded-lg p-6 w-96 shadow-lg relative">
        ...
        <div className="text-center text-red-500">
          {wrongInput === "blank" && "Please input Server Name!"}
          {wrongInput === "long" && "Please enter less than 20 characters !"}
        </div>
      </div>
      {/* {serverCategory && <div>Server Category: {serverCategory}</div>}
      {serverIcon && (
        <div>
          Server Icon: <img src={serverIcon} alt="Server Icon" />
        </div>
      )}
      {serverName && serverName} */}
    </div>
